...
from src.data.models import (
    ...
    CheckType, CheckDifficulty, CheckResult,
...
class GameEngine:
    ...
    def process_input(self, user_input: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if is_dialogue_turn:
                pending_dialogue_response = str(dm_output.response_to_player or "").strip()
                if not pending_dialogue_response:
                    pending_dialogue_response = self._build_dialogue_fallback_response(
                        natural_input,
                        dm_output=dm_output,
                    )
                if not dm_output.npc_response_needed and not has_player_action:
                    sanguo_progression = self._build_sanguo_wait_progression(
                        natural_input,
                        existing_changes=dialogue_supplemental_changes,
                    )
                    if sanguo_progression.get("changes") or sanguo_progression.get("fragments"):
                        ...
                        if dialogue_supplemental_changes:
                            ...
                            if failures:
                                ...
                                result["response"] = f"状态变更失败: {failures[0]}"
                                return result
                        result["response"] = pending_dialogue_response
                        self._record_dm_dialogue(natural_input, pending_dialogue_response)
                        return result
            ...
            if evolution_result is not None and sanguo_progression.get("changes"):
                evolution_result.changes.extend(sanguo_progression.get("changes", []))
            ...
            # 结局判定优先使用配置规则；仅当当前世界没有配置结局时才回退到 AI 复核。
            config_ending_text = self._evaluate_configured_endings()
            if evolution_result and evolution_result.is_end:
                self._is_game_over = True
                self._ending_text = evolution_result.end_narrative or config_ending_text
                result["game_over"] = True
                if self._ending_text:
                    result["narrative"] += f"\n\n{self._ending_text}"
            elif config_ending_text:
                self._is_game_over = True
                ...
                ai_end = None
                if hasattr(self.state_agent, "check_end_condition") and not self._ending_rules:
                    try:
                ...
                if ai_end and ai_end.is_end:
                    self._is_game_over = True
                    self._ending_text = ai_end.end_narrative
                    result["game_over"] = True
                    if self._ending_text:
                        result["narrative"] += f"\n\n{self._ending_text}"
    ...
    def _is_sanguo_demo_scene(self) -> bool:
        """Return True for the Three Kingdoms educational demo world."""
        return str(self.world_name or "").strip().lower() == "sanguo_mao_lu"

    def _known_key_facts(self) -> set[str]:
        """Return normalized narrative key facts currently known to the engine."""
        narrative_memory = self._narrative_memory_builder.build(self._dump_narrative_context())
    ...
    def _has_pending_field_change(
        ...
        for change in changes or []:
            if change.id == entity_id and change.field == field:
                return True
        return False

    def _build_sanguo_wait_progression(
        self,
        player_input: str,
        existing_changes: Optional[List[StateChange]] = None,
    ) -> Dict[str, Any]:
        """Advance the Sanguo cottage scene in two stages: report first, then admit."""
        progression: Dict[str, Any] = {"changes": [], "fragments": [], "key_facts": []}
        if not self._is_sanguo_demo_scene():
            return progression

        player = self.game_state.get_player()
        current_map = self.game_state.get_current_map()
        if not player or not current_map or player.location != "map-cottage-outer-01":
            return progression
        ...
        if re.search(rude_pattern, text):
            return progression
        if self._has_pending_field_change(existing_changes, player.id, "location"):
            return progression

        known_facts = self._known_key_facts()
        has_seen_tongzi = "\u5df2\u89c1\u4e66\u7ae5" in known_facts or "char-tongzi-01" in set(getattr(getattr(current_map, "entities", None), "characters", []) or [])
        if not has_seen_tongzi:
            return progression

        if "\u5df2\u901a\u62a5\u4e66\u7ae5" not in known_facts:
            if not self._has_pending_field_change(existing_changes, "char-tongzi-01", "location"):
                progression["changes"].append(
                    StateChange(
                        id="char-tongzi-01",
                        field="location",
                        operation=ChangeOperation.MOVE,
                        value="map-cottage-inner-01",
                    )
                )
            progression["fragments"].append(
                {
                    "actor_id": "char-tongzi-01",
                    "actor_name": "\u4e66\u7ae5",
                    "text": (
                        "\u89c1\u4f60\u786e\u5b9e\u613f\u610f\u5728\u95e8\u5916\u9759\u5019\uff0c\u4e66\u7ae5\u5411\u4f60\u4e00\u63d6\uff0c\u4f4e\u58f0\u9053\uff1a"
                        "\"\u5c06\u519b\u4e14\u5728\u6b64\u7a0d\u5019\uff0c\u7ae5\u5b50\u8fd9\u5c31\u5165\u5185\u79c9\u62a5\u5148\u751f\u3002\""
                        "\u8bf4\u7f62\uff0c\u4ed6\u4fbf\u8f7b\u624b\u8f7b\u811a\u5730\u63a9\u95e8\u5165\u5185\uff0c\u53ea\u7559\u4f60\u5728\u6885\u6811\u4e0b\u5b89\u9759\u7b49\u5019\u3002"
                    ),
                }
            )
            progression["key_facts"].extend(["\u5df2\u89c1\u4e66\u7ae5", "\u5df2\u901a\u62a5\u4e66\u7ae5"])
            return progression
        ...
        progression["fragments"].append(
            {
                ...
                "text": (
                    "\u4f60\u5728\u95e8\u5916\u53c8\u9759\u5019\u4e86\u4e00\u9635\uff0c\u59cb\u7ec8\u4e0d\u66fe\u50ac\u4fc3\u3002"
                    "\u5c11\u9877\uff0c\u67f4\u95e8\u8f7b\u8f7b\u4e00\u54cd\uff0c\u4e66\u7ae5\u4ece\u5185\u95f4\u5feb\u6b65\u8d70\u51fa\uff0c\u5411\u4f60\u884c\u4e86\u4e00\u793c\u3002"
                    "\u4ed6\u4f4e\u58f0\u9053\uff1a\"\u5148\u751f\u5df2\u9192\uff0c\u8bf7\u5c06\u519b\u5165\u5185\u3002\""
                    "\u4f60\u968f\u4ed6\u7f13\u6b65\u8fdb\u5165\u8305\u5e90\u5185\u5ba4\uff0c\u7ec8\u4e8e\u5f97\u4ee5\u5f53\u9762\u6c42\u89c1\u3002"
                ),
            }
        )
        progression["key_facts"].extend(["\u5df2\u89c1\u4e66\u7ae5", "\u5df2\u901a\u62a5\u4e66\u7ae5"])
    ...
    def _derive_turn_key_facts(self) -> List[str]:
        ...
        facts: List[str] = []
        seen = set()
        tongzi_report_pattern = re.compile(
            r"\u8f6c\u8eab.*?(?:\u67f4\u95e8|\u5165\u5185|\u95e8\u5185|\u5185\u95f4)|"
            r"\u5feb\u6b65.*?(?:\u67f4\u95e8|\u5165\u5185|\u95e8\u5185|\u5185\u95f4)|"
            r"\u8f7b\u624b\u8f7b\u811a.*?(?:\u5165\u5185|\u95e8\u5185|\u5185\u95f4)|"
            r"\u524d\u53bb\u901a\u62a5|\u51c6\u5907\u5165\u5185\u901a\u62a5|\u5165\u5185\u56de\u7980|\u63a9\u5e18\u800c\u51fa"
        )
        for step in list(getattr(self._current_turn_trace, "steps", []) or []):
            ...
            narrative_without_dialogue = re.sub(r"[\"'\u201c\u201d\u2018\u2019].*?[\"'\u201c\u201d\u2018\u2019]", "", local_narrative)
            if actor_name == "\u4e66\u7ae5" and tongzi_report_pattern.search(narrative_without_dialogue):
                report_fact = "\u5df2\u901a\u62a5\u4e66\u7ae5"
                if report_fact not in seen:
                    seen.add(report_fact)
                    facts.append(report_fact)
    ...
    def _process_unified_npc_response(
        ...
        for npc_id in ordered_npc_ids:
            ...
            if npc_output.is_end:
                if self._ending_rules:
                    logger.warning("当前世界已配置代码结局，忽略 NPC 阶段的 AI 终局判定")
                else:

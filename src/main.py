...
DEFAULT_WORLD = "sanguo_mao_lu"
APP_TITLE = "AI阅读实验室"
APP_SUBTITLE = "多世界沉浸式教育场景"


def get_app_text(world_name: str) -> tuple[str, str]:
    normalized = str(world_name or "").strip().lower()
    mapping = {
        "daiyu_enters_jia": ("AI阅读实验室", "《红楼梦·黛玉初进贾府》沉浸式教育场景"),
        "sanguo_mao_lu": ("AI阅读实验室", "《三国演义·三顾茅庐》沉浸式教育场景"),
    }
    return mapping.get(normalized, (APP_TITLE, APP_SUBTITLE))
...
def main():
    ...
    try:
        ...
        app_title, app_subtitle = get_app_text(args.world)
        logger.info("=" * 60)

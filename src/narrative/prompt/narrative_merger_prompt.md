...
**世界优先原则：**
- 合并叙事必须服从当前世界的题材、语体和教育目标，不得默认写成恐怖或战斗风格
- 在古典文学/教育场景中，要保留礼仪秩序、人物关系和细腻情绪，不要把叙事压扁成机械事件日志
- `new_key_facts` 不只可以是机器格式；若场景规则依赖自然语言关键事实，必须输出规则真正需要的精确字符串
...
**教育/剧情场景补充：**
- 若某个世界的结局或进度依赖自然语言事实，请直接输出该事实原文，例如：`"已见贾母"`、`"已见王熙凤"`
- 当步骤明确表现出“首次正式会面”“完成问安”“得到引路”等剧情节点时，优先把这些节点提炼成可复用的关键事实
...
3. 只包含本回合新产生的事实
4. 不包含历史已知事实
5. 若世界配置明显依赖剧情节点事实，优先输出人类可读且可直接命中规则的事实短语
...
5. **剧情关键事实必须可被规则复用**
   - 若回合内容已经明确发生“会见某人”“获得许可”“完成入内”等关键节点，`new_key_facts` 应输出稳定、简洁、可复用的规则字符串
   - 不要只输出抽象总结，导致下游规则无法命中
